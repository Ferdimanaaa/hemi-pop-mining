...
if [ "$pilihan" == "1" ]; then
    ...
    if [[ "$tersimpan" =~ ^[Yy]$ ]]; then
        ...
        if [[ "$faucet_diminta" =~ ^[Yy]$ ]]; then
            ...
            screen -dmS mgsky ./popmd
            if [ $? -ne 0 ]; then
                tampilkan "Gagal memulai PoP mining dalam sesi screen terpisah."
                exit 1
            fi

            tampilkan "PoP mining telah dimulai dalam sesi screen terpisah bernama 'mgsky'."
        fi
    fi
...
echo -e "${BOLD_PINK} Bergabung dengan mgsky testnet hunter https://chat.whatsapp.com/LbFjrYKCaRr0fW4gPvcKCj ${RESET_COLOR}"
